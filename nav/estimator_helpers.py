...
class Estimator():
    ...
    def estimate_relative_pose(self, sensor_image, start_state, sig, obs_img_pose=None):
        ...
        for k in range(self.iter):
            optimizer.zero_grad()
            # rand_inds = np.random.choice(interest_regions.shape[0], size=self.batch_size, replace=False)    # Original

            num_available = interest_regions.shape[0]
            # Use the batch size if enough features exist, otherwise use all available features
            actual_sample_size = min(self.batch_size, num_available)

            if num_available > 0:
                rand_inds = np.random.choice(num_available, size=actual_sample_size, replace=False)
            else:
                # If 0 features are found, we cannot estimate the pose relative to the image
                success_flag = False
                return self.xt, success_flag # Modified
    ...
    def estimate_state(self, sensor_img, obs_img_pose, action):
        ...
        self.action = action.clone().detach().cpu().numpy().tolist()
